...
def upload_cars():
    ...
    Button(upload_car_screen, text="Submit car", font=("calibri", 20), width=20, height=1, bg="yellow", relief="solid",
           command=submit_car_button).pack()
...
def client_menu():
    client_screen = Toplevel(main_window)
    client_screen.title('Client Screen')
    client_screen.geometry('1000x700')
    client_screen.configure(background='white')

    label1: Label = Label(client_screen, text="Search Car",bg=("yellow"),font=("calibri", 20, "bold"))
    label1.place(x=450, y=110)
    global enter_box
    enter_box = Entry(width=30)
    enter_box.place(x=420, y=154)

    btn = Button(client_screen,text="Search",command=get_search,bg="yellow",fg="white",font=("calibri", 10, "bold"))
    btn.place(x=325, y=150)

    photo1 = PhotoImage(file="/home/athanpan/Έγγραφα/FindYourCar/FindYourCar.png")
    varun_label = Label(image=photo1)
